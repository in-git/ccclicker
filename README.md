...
**注意：首次构建可能需要 10-15 分钟，请耐心等待。**

### 📦 构建输出

构建完成后，签名的文件位于：
- `src-tauri/target/release/bundle/msi/连点点_1.0.1_x64_zh-CN.msi`
- `src-tauri/target/release/bundle/msi/连点点.exe`

### 🔐 代码签名说明

为了解决浏览器下载时的"不安全"警告，项目集成了代码签名功能：

- **测试环境**：首次运行会自动创建自签名证书（仅用于测试）
- **生产环境**：购买商业代码签名证书，替换 `cert.pfx` 文件即可

### 🛠️ 手动命令（可选）

| 命令 | 功能 |
|------|------|
| `npm run build:release` | 完整构建流程 |
| `npm run setup-signing` | 设置签名环境 |
